    ...
    public class ConfirmOnlinePaymentCommandHandler : IRequestHandler<ConfirmOnlinePaymentCommand, Unit>
    {
        ...
        public async Task<Unit> Handle(ConfirmOnlinePaymentCommand request, CancellationToken cancellationToken)
        {
            ...
            if (transaction is null)
                throw new KeyNotFoundException($"No payment transaction found with gateway reference '{request.GatewayReference}'.");

            if (transaction.Status != PaymentTransactionStatus.Pending)
                throw new InvalidOperationException($"This transaction has already been processed with status '{transaction.Status}'.");
